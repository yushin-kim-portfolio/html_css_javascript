...
$(document).ready(function() {
  ...
  // Icon 표시 동시 출력 -- 페이지 추가시 수정요망
  setTimeout(function() {
    $('#move1page').animate({left : '48px', opacity : '1'}, 800);
  }, 1600);
  setTimeout(function() {
    $('#move2page').animate({left : '48px', opacity : '1'}, 800);
  }, 1600);
  setTimeout(function() {
    $('#move3page').animate({left : '48px', opacity : '1'}, 800);
  }, 1600);
  //로고 / 번호표 표시
  setTimeout(function() {
...
$("html").on("mousewheel", function(event) {
  if (event.originalEvent.wheelDelta < 0 && mouseWheelEvent < 3) {
    mouseWheelEvent++;
  ...
  if (mouseWheelEvent == 0) {
    moveOnePage();
  } else if (mouseWheelEvent == 1) {
    moveTwoPage();
  } else if (mouseWheelEvent == 2) {
    moveThreePage();
  }
});
